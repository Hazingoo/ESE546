...
def train_model(model, train_data, val_data, char_map, epochs=50, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(train_seqs), batch_size):
            ...
            outputs, _ = model(batch_seqs, hidden)
            
            loss = criterion(outputs.view(-1, model.vocab_size), batch_targets)
            loss.backward()
        ...
        model.eval()
        with torch.no_grad():
            val_hidden = model.init_hidden(val_seqs.size(0))
            val_outputs, _ = model(val_seqs, val_hidden)
            val_loss = criterion(val_outputs.view(-1, model.vocab_size), val_targets)
            
            _, val_predicted = torch.max(val_outputs, 2)
            val_predicted = val_predicted.view(-1)
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch}: Train Loss={avg_loss:.4f}, Train Error={avg_error:.4f}, Val Loss={val_loss.item():.4f}, Val Error={val_error.item():.4f}")
...
if __name__ == '__main__':
    ...
    # Train
    train_losses, train_errors, val_losses, val_errors = train_model(
        model, train_data, val_data, char_map, epochs=50, lr=1e-3
    )
